...
from src.utils.dat_to_cst import CST_fitting
import tempfile
import os
import shutil
...
async def get_cst_values(file: UploadFile = File(...), bernenstein_order: int = 8):
        temp_file_path = None
        ...
        try:
            ...
            print(f"Received file: {file.filename} with Bernstein order: {bernenstein_order}")
            ...
            try:
                output = CST_fitting(temp_file_path, output_file_path, bernenstein_order)
                return {
                ...
                raise HTTPException(status_code=500, detail=f"Fitting algorithm failed: {str(e)}")
