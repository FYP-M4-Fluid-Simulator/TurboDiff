...
def run_aoa(aoa_deg: float):
    ...
    # ── Simulation loop ──────────────────────────────────────────────────────
    print(
        "\n  Running simulation with visualisation (calculating forces at the end)..."
    )
    converged = False
    ...
    for step in range(1, N_STEPS + 1):
        ...
        # PyGame Visualisation / Event Pump
        if grid.visualise:
        ...
        if step % REPORT_EVERY == 0:
            ...
            print(f"  [{step:>6} / {N_STEPS}]  elapsed: {elapsed:.1f}s")

    print("  ✓ Simulation finished. Calculating aerodynamic forces...")

    # Final forces calculated just once at the very end
    nu_eff = grid.compute_effective_viscosity(state)
