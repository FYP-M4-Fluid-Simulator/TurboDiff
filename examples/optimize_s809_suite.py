        ...
        for aoa_val in AOA_LIST:
            ...
            for i in range(NUM_ITERATIONS):
                ...
                if xf:
                    ...
                    # Check thickness constraints for validity
                    x_c, yu_c, yl_c = generate_cst_coords(wu, wl, 200)
                    t_max_curr = float(max_thickness(yu_c, yl_c))
                    thickness_ok = (
                        MIN_THICKNESS_RATIO <= t_max_curr <= MAX_THICKNESS_RATIO
                    )

                    # Update best airfoil if Xfoil L/D improves and thickness is within bounds
                    if eff > best_cl_cd and thickness_ok:
                        best_loss = float(val)
                    ...
                    print(f"    Iter {i:2d}: Loss={float(val):.4f}, Xfoil failed")
